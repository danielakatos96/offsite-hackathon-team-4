...
async function getData() {
  // This function is kept for compatibility but now returns empty data
  // The actual search is handled by searchDocuments function
...
export default async function SearchPage({ searchParams }) {
  ...
  if (q) {
    const searchResponse = await searchDocuments(q, 20);
    ...
    <div className="min-h-screen bg-background text-foreground p-6">
      <div className="max-w-5xl mx-auto flex flex-col items-center gap-8">
        ...
        <form
          action="/search"
          method="GET"
          className="w-full max-w-xl mx-auto flex items-stretch gap-2"
        >
          <input
            type="search"
            name="q"
              ...
              >
                <div className="p-5 flex flex-col gap-3">
                  ...
                  <p
                    ...
                    dangerouslySetInnerHTML={{
                      __html: highlight(excerpt(doc.text, q), q),
                    }}
                  />
